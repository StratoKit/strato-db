declare module 'strato-db' {
	...
	export interface DB extends SQLite {
		...
		): InstanceType<DBModel>
		/**
		 * Register an object with migrations.
		 * Migrations are marked completed by the given name + their name in the
		 * `{sdb}
		 * migrations` table.
		 *
	...
	> {
		...
		parseRow: (row: SQLiteRow, options?: SearchOptions) => JMItem
		/**
		 ...
		 */
		searchAll(attrs: SearchAttrs, options?: SearchOptions): Promise<JMItem[]>
		/**
		 ...
		 *
		 * @returns - the objects, or null where they don't exist, in order of
		 *          their requested ID.
		 */
		getAll(
		 ...
		 */
		each(cb: ItemCallback<JMItem>): Promise<void>
		each(attrs: SearchAttrs, cb: ItemCallback<JMItem>): Promise<void>
		each(
			attrs: SearchAttrs,
			opts: SearchOptions,
			cb: ItemCallback<JMItem>
		): Promise<void>
		/**
		 ...
		 */
		update(obj: JMItem, upsert?: boolean, noReturn?: boolean): Promise<JMItem>
		/**
		 ...
		 */
		remove(idOrObj: IDType | JMItem): Promise<void>
		/**
		 * "Rename" an object.
		 *
		...
		implements ESDBModel
	{
		 ...
		 */
		remove(idOrObj: EMItem | IDValue, meta?: any): Promise<void>
		/**
