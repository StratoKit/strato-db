...
export default function createStore(
	...
	function dispatch(action) {
		...
		try {
			isDispatching = true
			if (isAsync) {
				// Promise rejections passed on
				// reducers should not throw for any action, only real errors
				// that need to be handled by the app
				return currentReducer(currentState, action).then(newState => {
					currentState = newState
					return finishDispatch(action)
				})
			}
	...
	const didInitialize = dispatch({type: ActionTypes.INIT})
