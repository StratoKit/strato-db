...
const prepareSqlCol = (col, name) => {
  ...
  if (!col.sql) {
    col.sql = col.real ? `tbl.${col.quoted}` : `json_extract(tbl.${sql.quoteId(col.jsonCol)},'$.${col.jsonPath}')`;
  }
  if (col.isAnyOfArray) {
    col.isArray = true;
    col.in = true;
  ...
  if (col.isArray) {
    ...
    const eachSql = `json_each(${col.real ? `tbl.${col.quoted}` : `tbl.${sql.quoteId(col.jsonCol)},'$.${col.jsonPath}'`})`;
    if (col.in) {
