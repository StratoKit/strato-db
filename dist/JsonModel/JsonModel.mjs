...
import "../DB/DB.mjs";
import { sql } from "../DB/SQLite.mjs";
import DataLoader from "dataloader";
import { get, set } from "lodash";
import { normalizeColumn } from "./normalizeColumn.mjs";
import { assignJsonParents } from "./assignJsonParents.mjs";
import { parseJson, stringifyJsonObject, prepareSqlCol, byPathLength, byPathLengthDesc } from "./prepareSqlCol.mjs";
import { verifyOptions, verifyColumn } from "./verifyOptions.mjs";
import { makeMigrations } from "./makeMigrations.mjs";
import { makeIdValue } from "./makeDefaultIdValue.mjs";
...
class JsonModelImpl {
  ...
  _makeSetFn() {
    ...
    const realCols = columnArr.filter((c) => c.real).sort(byPathLengthDesc).map((c, i) => ({
      ...c,
      i,
  ...
  delete(idOrObj) {
    if (DEV)
      deprecated("deleteMethod", "use .remove() instead of .delete()");
