...
const ESModel = require("./ESModel.js");
...
class EventSourcingDB extends EventEmitter {
  ...
  constructor({
    ...
    this.db = this.rwDb.file === ":memory:" || readOnly ? this.rwDb : new DB.default({
      ...dbOptions,
    ...
    this._resultQueue = this.rwDb.file === qDbFile && qDbFile !== ":memory:" ? new EventQueue({ db: this.rwDb }) : this.queue;
    ...
    this.store = {};
    this.rwStore = {};
    this._alsDispatch = new node_async_hooks.AsyncLocalStorage();
    this._preprocModels = [];
    this._reducerNames = [];
    this._deriverModels = [];
        ...
        if (RWModel === ESModel.default) {
          ...
          if (preprocessor) {
            ...
            preprocessor = async (args) => {
              const e = await ESModel.default.preprocessor(args);
              if (e)
                args.event = e;
              return prev(args);
            };
  ...
  async _preprocessor(cache, event, isMainEvent) {
    ...
    const dispatch = (...args) => {
      warning.deprecated("addEvent preprocessor", "use .addEvent instead of .dispatch");
  ...
  async _reducer(cache, event, isMainEvent) {
    ...
    await Promise.all(
      this._reducerNames.map(async (name) => {
        ...
        const addEvent = this._makeAddSubEvent(event);
        const dispatch = (...args) => {
          warning.deprecated("addEvent", "use .addEvent instead of .dispatch");
          return addEvent(...args);
        };
        const helpers = {
          cache,
  ...
  async _applyEvent(event, isMainEvent) {
    ...
    try {
      ...
      if (!event.error && _deriverModels.length) {
        ...
        const addEvent = this._makeAddSubEvent(event);
        const dispatch = (...args) => {
          warning.deprecated("addEvent deriver", "use .addEvent instead of .dispatch");
          return addEvent(...args);
        };
        await settleAll.settleAll(
          this._deriverModels,
...
module.exports = EventSourcingDB;
