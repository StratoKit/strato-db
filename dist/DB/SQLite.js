...
exports.default = exports.sql = exports.valToSql = void 0;
...
const dbg = (0, _debug.default)('strato-db/sqlite');
...
const getDuration = ts => (_perf_hooks.performance.now() - ts).toLocaleString(undefined, {
  maximumFractionDigits: 2
});

const objToString = o => {
  const s = (0, _util.inspect)(o, {
    compact: true,
...
class SQLite extends _events.EventEmitter {
  /**
   * @constructor
  ...
  constructor(_ref = {}) {
    let {
      ...
      _statements = {}
        ...
        rest = _objectWithoutProperties(_ref, ["file", "readOnly", "verbose", "onWillOpen", "onDidOpen", "autoVacuum", "vacuumInterval", "vacuumPageCount", "name", "_sqlite", "_store", "_statements"]);

    super();

    _defineProperty(this, "sql", sql);

    _defineProperty(this, "transactionP", Promise.resolve());
  ...
  async _openDB() {
    ...
    if (_isChild) throw new Error(`Child dbs cannot be opened. Perhaps you kept a prepared statement from a child db?`);
    if (onWillOpen) await onWillOpen();
    dbg(`${this.name} opening ${this.file}`);

    const _sqlite = await new Promise((resolve, reject) => {
      if (verbose) _sqlite2.default.verbose();
      const mode = readOnly ? _sqlite2.default.OPEN_READONLY : _sqlite2.default.OPEN_READWRITE | _sqlite2.default.OPEN_CREATE;
      const db = new _sqlite2.default.Database(file, mode, err => {
        if (err) reject(new Error(`${file}: ${err.message}`));else resolve(db);
      });
    }); // Wait for locks


    _sqlite.configure('busyTimeout', process.env.NODE_ENV === 'test' ? 10 : 1000 + Math.floor(Math.random() * 500));
    ...
    if (!this.readOnly) {
      ...
      if (this.file !== ':memory:') {
        ...
        if (mode !== 'wal') {
          ...
          } = await childDb.get('PRAGMA journal_mode = wal').catch(err => {
            if (!isBusyError(err)) throw err;
          });

          if (journalMode !== 'wal') {
            // eslint-disable-next-line no-console
            console.error(`!!! WARNING: journal_mode is ${journalMode}, not WAL. Locking issues might occur!`);
          }
        }
      }
      ...
      if (autoVacuum) {
        const {
          auto_vacuum: mode
        } = await childDb.get(`PRAGMA auto_vacuum`);
        ...
        this._vacuumToken = setInterval(() => this._vacuumStep(), vacuumInterval * 10 * 1000);
      ...
      await childDb.exec(`
				...
				`);
      this._optimizerToken = setInterval(() => this.exec(`PRAGMA optimize`), 2 * 3600 * 1000);
  ...
  async close() {
    ...
    this._sqlite = null; // eslint-disable-next-line no-await-in-loop

    for (const stmt of Object.values(this.statements)) await stmt.finalize(); // We only want to close our own statements, not the db
  ...
  async _call(method, args, obj, name, returnThis, returnFn) {
    ...
    if (!obj) _sqlite = await this._hold(method);else if (isStmt) _sqlite = obj._stmt;else _sqlite = obj;
    if (!_sqlite) throw new Error(`${name}: sqlite or statement not initialized`); // Template strings
    ...
    const now = dbgQ.enabled ? _perf_hooks.performance.now() : undefined;
    let fnResult;
    const result = new Promise((resolve, reject) => {
      ...
      let busyRetry = RETRY_COUNT; // We need to consume `this` from sqlite3 callback
      ...
      cb = function (err, out) {
        if (err) {
          ...
          const error = new Error(`${name}: sqlite3: ${err.message}`); // @ts-ignore

          error.code = err.code;
          reject(error);
        } else resolve(returnFn ? fnResult : returnThis ? {
          lastID: this.lastID,
          changes: this.changes
        } : out);
      };

      if (!_sqlite[method]) return cb({
    ...
    if (dbgQ.enabled) {
      ...
      const v = args ? isStmt ? objToString(args) : objToString(args[isStmt ? 0 : 1]) : ''; // eslint-disable-next-line promise/catch-or-return

      result.then(o => {
        if (returnFn) o = fnResult;
        const d = getDuration(now);
        const out = method !== 'exec' && method !== 'prepare' ? `-> ${objToString(o)}` : '';
  ...
  prepare(sql, name) {
    ...
    return new _Statement.default(this, sql, name);
  }
  /**
  ...
  async dataVersion() {
    ...
    if (!this._dataVSql) this._dataVSql = this.prepare('PRAGMA data_version', 'dataV');
    const {
      data_version: v
    } = await this._dataVSql.get();
    return v;
  }
  /**
  ...
  async userVersion(newV) {
    if (!this._sqlite) await this._hold('userVersion'); // Can't prepare or use pragma with parameter

    if (newV) return this._call('exec', [`PRAGMA user_version=${Number(newV)}`], this._sqlite, this.name);
    if (!this._userVSql) this._userVSql = this.prepare('PRAGMA user_version', 'userV');
    const {
      user_version: v
    } = await this._userVSql.get();
    return v;
  }
  ...
  async withTransaction(fn) {
    ...
    if (!this._sqlite) await this._hold('transaction'); // Prevent overlapping transactions in this process
...
_defineProperty(SQLite, "sql", sql);
